...
class BaseAlgorithm(ABC):
    ...
    def _update_learning_rate(self, optimizers: Union[List[th.optim.Optimizer], th.optim.Optimizer]) -> None:
        ...
        if isinstance(self.lr_schedule, list):
            lr_list = self.lr_schedule
        else:
            self.logger.record("config/learning_rate", self.lr_schedule(self._current_progress_remaining))
            lr_list = [self.lr_schedule]

        if not isinstance(optimizers, list):
            optimizers = [optimizers]
        for optimizer, schedule in zip(optimizers, lr_list):
            update_learning_rate(optimizer, schedule(self._current_progress_remaining))
    ...
    def _setup_learn(
        ...
        # new, but why not update logger only on start?
        if not self._custom_logger and self._logger is None:
            self._logger = utils.configure_logger(self.verbose, self.tensorboard_log, tb_log_name, reset_num_timesteps)
    ...
    def set_parameters(
        ...
        if torch_save_params_only is None:
            objects_needing_update = set(self._get_torch_save_params()[0])
        else:
            objects_needing_update = torch_save_params_only
        updated_objects = set()
    ...
    def get_parameters(self, torch_save_params=None) -> Dict[str, Dict]:
        """
        ...
        """
        if torch_save_params is not None:
            state_dicts_names, _ = torch_save_params
        else:
    ...
    def save(
        ...
        params_to_save = self.get_parameters((state_dicts_names, torch_variable_names))
