...
class RolloutBuffer(BaseBuffer):
    ...
    def reset(self) -> None:
        ...
        self.observations = np.zeros((self.max_steps, self.n_envs) + self.obs_shape, dtype=np.float32)
    ...
    def get(self, batch_size: Optional[int] = None, seed: Optional[int] = None) -> Generator[RolloutBufferSamples, None, None]:
        assert self.full, ""
        if seed:
            np.random.seed(seed)
...
class RolloutBufferMC(BaseBuffer):
    """
    Rollout buffer used in on-policy algorithms like A2C/PPO.
    It corresponds to ``buffer_size`` transitions collected
    using the current policy.
    ...
    def __init__(
        ...
        super(RolloutBufferMC, self).__init__(buffer_size, observation_space, action_space, device, n_envs=n_envs)
        self.gae_lambda = gae_lambda
        self.gamma = gamma
        self.observations, self.actions, self.rewards, self.advantages = None, None, None, None
        self.returns, self.episode_starts, self.values, self.log_probs = None, None, None, None
    ...
    def reset(self) -> None:
        ...
        self.done_indices = np.arange(self.n_envs)
        self.episodes_done = False
        super(RolloutBufferMC, self).reset()
    ...
    def compute_returns_and_advantage(self, dones: np.ndarray) -> None:
        ...
        for step in range(self.max_steps - 1, -1, -1):
            if step == self.max_steps - 1:
                ...
                # next_values = self.values[step + 1]
            # delta = self.rewards[step] + self.gamma * next_values * next_non_terminal - self.values[step]
            last_gae_lam = self.rewards[step] + self.gamma * self.gae_lambda * next_non_terminal * last_gae_lam
            self.advantages[step] = last_gae_lam
