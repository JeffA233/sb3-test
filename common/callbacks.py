...
from gym import Env
# import numpy as np
from numpy import inf, savez, mean, sum, std
...
class BaseCallback(ABC):
    ...
    def __init__(self, verbose: int = 0, start_count: int = 0):
        super(BaseCallback, self).__init__()
        # The RL model
        self.model = None  # type: Optional[base_class.BaseAlgorithm]
        # An alias for self.model.get_env(), the environment used for training
        self.training_env = None  # type: Union[Env, VecEnv, None]
        # Number of time the callback was called
        self.n_calls = start_count  # type: int
...
class CheckpointCallback(BaseCallback):
    ...
    def __init__(self, save_freq: int, save_path: str, name_prefix: str = "rl_model", verbose: int = 0,
                 start_count: int = 0, exclusions=None):
        super(CheckpointCallback, self).__init__(verbose)
        self.exclusions = exclusions
        self.n_calls = start_count
    ...
    def _on_step(self) -> bool:
        if self.n_calls % self.save_freq == 0:
            path = join(self.save_path, f"{self.name_prefix}_{self.num_timesteps}_steps")
            if self.exclusions is not None:
                self.model.save(path, exclude=self.exclusions)
            else:
...
class EvalCallback(EventCallback):
    ...
    def __init__(
        ...
        if log_path is not None:
            log_path = join(log_path, "evaluations")
    ...
    def _init_callback(self) -> None:
        ...
        if not isinstance(self.training_env, type(self.eval_env)):
            warn("Training and eval env are not of the same type" f"{self.training_env} != {self.eval_env}")

        # Create folders if needed
        if self.best_model_save_path is not None:
            makedirs(self.best_model_save_path, exist_ok=True)
        if self.log_path is not None:
            makedirs(dirname(self.log_path), exist_ok=True)
    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            mean_reward, std_reward = mean(episode_rewards), std(episode_rewards)
            mean_ep_length, std_ep_length = mean(episode_lengths), std(episode_lengths)
            ...
            if len(self._is_success_buffer) > 0:
                success_rate = mean(self._is_success_buffer)
                if self.verbose > 0:
            ...
            if mean_reward > self.best_mean_reward:
                ...
                if self.best_model_save_path is not None:
                    self.model.save(join(self.best_model_save_path, "best_model"))
...
class StopTrainingOnMaxEpisodes(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        self.n_episodes += sum(self.locals["dones"]).item()
