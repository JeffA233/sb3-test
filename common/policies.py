...
class ActorCriticPolicySkip(BasePolicy):
    """
    ...
    def __init__(
        ...
        super(ActorCriticPolicySkip, self).__init__(
            observation_space,
            action_space,
    ...
    def _build_mlp_extractor(self) -> None:
        ...
        self.mlp_extractor = MlpExtractorSkip(
            self.features_dim,
            net_arch=self.net_arch,
            activation_fn=self.activation_fn,
            device=self.device,
    ...
    def _build(self, lr_schedule: Schedule) -> None:
        ...
        if self.ortho_init:
            ...
            if isinstance(self.activation_fn, nn.LeakyReLU):
                print("using LeakyReLU values")
                module_gains = {
                    self.features_extractor: np.sqrt(2),
                    self.mlp_extractor: np.sqrt(2),
                    # recommended gain for LeakyReLU
                    self.action_net: np.sqrt(2/(1+1e-2**2)),
                    self.value_net: np.sqrt(2/(1+1e-2**2)),
                }
...
class ActorCriticPolicy(BasePolicy):
    ...
    def __init__(
        ...
        self._build(lr_schedule, symlog, device)
    ...
    @th.jit.ignore
    def _build_mlp_extractor(self, symlog: bool) -> None:
        """
        Create the policy and value networks.
        Part of the layers can be shared.
        """
    ...
    def _build(self, lr_schedule: Schedule, symlog: bool, device_str: str) -> None:
        ...
        self._build_mlp_extractor(symlog)
        ...
        if self.ortho_init:
            ...
            module_gains = {
                ...
                # self.action_net: np.sqrt(2 / (1 + 1e-2 ** 2)),
                # self.value_net: np.sqrt(2 / (1 + 1e-2 ** 2)),
                self.action_net: action_net_init,
                self.value_net: value_net_init,
            }
            for module, gain in module_gains.items():
                module.apply(partial(self.init_weights, gain=gain))

            # for params in self.mlp_extractor.policy_net.modules():
            #     # params.apply(partial(self.init_weights, gain=action_net_init))
            #     self.init_weights(params, gain=action_net_init)
            #
            # for params in self.mlp_extractor.value_net.modules():
            #     # params.apply(partial(self.init_weights, gain=action_net_init))
            #     self.init_weights(params, gain=value_net_init)
    ...
    # @th.jit.export
    # def get_act_and_logprob(self, obs: th.Tensor) -> (th.Tensor, th.Tensor):
    #     """
    #     Get the current policy distribution given the observations.
    #
    #     :param obs:
    ...
    #     mean_actions = self.action_net(latent_pi)
    #
    #     probs = mean_actions.softmax(-1)
    #     probs = probs.view(-1, self.action_space.n)
    #
    #     action = th.multinomial(probs, 1, True)
    #     log_prob = probs.log().gather(-1, action)
    #
    #     return action.flatten(), log_prob.flatten()

        # return self._get_action_dist_from_latent(latent_pi)

    def get_latent(self, obs: th.Tensor) -> th.Tensor:
        features = self.extract_features(obs)
...
class ActorCriticPolicyOptim(BasePolicy):
    """
    Policy class for actor-critic algorithms (has both policy and value prediction).
    Used by A2C, PPO and the likes.
    ...
    def __init__(
        ...
        super(ActorCriticPolicyOptim, self).__init__(
            observation_space,
            action_space,
            features_extractor_class,
            features_extractor_kwargs,
    ...
    def _build(self, lr_schedule: Schedule, symlog: bool, device_str: str) -> None:
        ...
        # value_params = [param for param in self.value_net.parameters()]
        # value_params.extend(self.mlp_extractor.value_net.parameters())
        if isinstance(lr_schedule, list):
            value_params = [param for param in self.value_net.parameters()]
            value_params.extend(self.mlp_extractor.value_net.parameters())
            self.value_optimizer = self.optimizer_class(value_params, lr=lr_schedule[0](1),
                                                        **self.optimizer_kwargs)

            act_params = [param for param in self.action_net.parameters()]
            act_params.extend(self.mlp_extractor.policy_net.parameters())
            self.policy_optimizer = self.optimizer_class(act_params, lr=lr_schedule[1](1),
                                                         **self.optimizer_kwargs)
        else:
            value_params = [param for param in self.value_net.parameters()]
            value_params.extend(self.mlp_extractor.value_net.parameters())
            self.value_optimizer = self.optimizer_class(value_params, lr=lr_schedule(1),
                                                        **self.optimizer_kwargs)

            act_params = [param for param in self.action_net.parameters()]
            act_params.extend(self.mlp_extractor.policy_net.parameters())
            self.policy_optimizer = self.optimizer_class(act_params, lr=lr_schedule(1),
                                                         **self.optimizer_kwargs)
    ...
    @th.jit.ignore
    def get_entr_prob(self, obs: th.Tensor, actions: th.Tensor) -> Tuple[th.Tensor, th.Tensor]:
        """
        Evaluate actions according to the current policy,
        given the observations.
...
class ACPolicyOptimSpecNorm(BasePolicy):
    """
    Policy class for actor-critic algorithms (has both policy and value prediction).
    Used by A2C, PPO and the likes.
    ...
    def __init__(
        ...
        super(ACPolicyOptimSpecNorm, self).__init__(
            observation_space,
            action_space,
            features_extractor_class,
            features_extractor_kwargs,
    ...
    def _build(self, lr_schedule: Schedule, symlog: bool, device_str: str) -> None:
        ...
        # for i, layer in enumerate(self.mlp_extractor.policy_net):
        #     if i > 1:
        #         if isinstance(layer, nn.Linear):
        #             nn.utils.parametrizations.spectral_norm(layer)
...
class ActorCriticPolicyNorm(BasePolicy):
    """
    Policy class for actor-critic algorithms (has both policy and value prediction).
    Used by A2C, PPO and the likes.
    ...
    def __init__(
        ...
        super(ActorCriticPolicyNorm, self).__init__(
            observation_space,
            action_space,
            features_extractor_class,
            features_extractor_kwargs,
    ...
    def _build_mlp_extractor(self) -> None:
        ...
        #       net_arch here is an empty list and mlp_extractor does not
        #       really contain any layers (acts like an identity module).
        self.mlp_extractor = MlpExtractorNorm(
            self.features_dim,
            net_arch=self.net_arch,
            activation_fn=self.activation_fn,
            device=self.device,
