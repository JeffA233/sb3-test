...
class RolloutBuffer(BaseBuffer):
    ...
    def reset(self) -> None:
        ...
        self.observations = np.zeros((self.buffer_size*3, self.n_envs) + self.obs_shape, dtype=np.float32)
        self.actions = np.zeros((self.buffer_size*3, self.n_envs, self.action_dim), dtype=np.float32)
        self.rewards = np.zeros((self.buffer_size*3, self.n_envs), dtype=np.float32)
        self.returns = np.zeros((self.buffer_size*3, self.n_envs), dtype=np.float32)
        self.episode_starts = np.zeros((self.buffer_size*3, self.n_envs), dtype=np.float32)
        self.values = np.zeros((self.buffer_size*3, self.n_envs), dtype=np.float32)
        self.log_probs = np.zeros((self.buffer_size*3, self.n_envs), dtype=np.float32)
        self.advantages = np.zeros((self.buffer_size*3, self.n_envs), dtype=np.float32)
        self.generator_ready = False
        self.pos_value = 0
        self.batch_size = 0
    ...
    def compute_returns_and_advantage(self, last_values: th.Tensor, dones: np.ndarray) -> None:
        ...
        # self.returns = self._numba_advantage_return(last_values, self.values, self.episode_starts, dones, self.rewards,
        #                                        self.gamma, self.buffer_size, self.gae_lambda)
    ...
    def add(
        ...
        if len(self.done_indices) == 0 or self.pos == self.buffer_size*3 - 1:
...
class PrioritizedExperienceReplay(BaseBuffer):
    ...
    def get(self, batch_size: Optional[int] = None) -> Generator[RolloutBufferSamples, None, None]:
        ...
        exp = np.exp(np.clip(self.value_est_err, -10, 10))
