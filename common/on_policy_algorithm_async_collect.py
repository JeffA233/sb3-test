...
class OnPolicyAlgorithm(BaseAlgorithm):
    ...
    def _setup_model(self) -> None:
        ...
        buffer_cls = DictRolloutBuffer if isinstance(self.observation_space, Dict_Space) else RolloutBuffer
    ...
    def collect_rollouts(
        ...
        # local function to be able to use a thread
        def infer_nets(queue_loc: queue.SimpleQueue, obs):
            obs_tensor_loc = obs_as_tensor(obs, self.device)
            with no_grad():
                dist = self.policy.get_distribution(obs_tensor_loc)
                acts = dist.get_actions()
                log_prbs = dist.log_prob(acts)
                acts = acts.cpu().numpy()
            if isinstance(self.action_space, Box):
                acts.clip(self.action_space.low, self.action_space.high, out=acts)
            queue_loc.put((log_prbs, acts))

        t2 = threading.Thread(target=infer_nets, args=(que, self._last_obs))
        t2.start()
        ...
        while n_steps < n_rollout_steps:
            ...
            t2.join()
            log_probs, actions = que.get()

            new_obs, rewards, dones, infos = env.step(actions)

            t2 = threading.Thread(target=infer_nets, args=(que, new_obs))
            t2.start()
