...
# import platform
from platform import platform, version, python_version
...
def explained_variance_torch(y_pred: th.Tensor, y_true: th.Tensor) -> th.Tensor:
    """
    Computes fraction of variance that ypred explains about y.
    Returns 1 - Var[y-ypred] / Var[y]
...
def get_schedule_fn(value_schedule: Union[Schedule, float, int, List[float]]) -> Schedule:
    """
    ...
    elif isinstance(value_schedule, list):
        value_schedule = [constant_fn(float(value)) for value in iter(value_schedule)]
    else:
        # for a case of multiple optimizers
...
def get_latest_run_id(log_path: Optional[str] = None, log_name: str = "") -> int:
    ...
    """
    max_run_id = 0
    for path in glob(f"{log_path}/{log_name}_[0-9]*"):
...
def configure_logger(
    ...
    if tensorboard_log is not None and SummaryWriter is not None:
        ...
        if not reset_num_timesteps:
            ...
            latest_run_id -= 1
        save_path = join(tensorboard_log, f"{tb_log_name}_{latest_run_id + 1}")
        if verbose >= 1:
...
def safe_max(arr: Union[np.ndarray, list, deque]) -> np.ndarray:
    """
...
def safe_min(arr: Union[np.ndarray, list, deque]) -> np.ndarray:
    """
...
def obs_as_tensor(
    obs: Union[np.ndarray, Dict[Union[str, int], np.ndarray]], device: th.device, dtype: th.dtype = th.float32
    ...
    if isinstance(obs, np.ndarray):
        ...
        return th.from_numpy(obs).to(dtype=dtype, device=device, non_blocking=True, copy=False)
    elif isinstance(obs, dict):
        return {key: th.as_tensor(_obs).to(dtype=dtype, device=device, non_blocking=True) for (key, _obs) in obs.items()}
    else:
...
def flat_grad(
    output,
    parameters: Sequence[th.nn.parameter.Parameter],
