...
# import torch as th
from torch import Tensor, device, minimum, clamp, abs, exp, pow, no_grad, mean, where, concatenate as th_concatenate, var, max, min as th_min, dist, topk, zeros as th_zeros, tensor, float32 as th_float32, sum as th_sum, isnan as th_isnan
...
class SPO_Optim(OnPolicyAlgorithm):
    """
    ...
    def __init__(
        ...
        self.weight_clip_k = weight_clip_k
    ...
    def _setup_model(self) -> None:
        super(SPO_Optim, self)._setup_model()
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.n_epochs_critic):
            ...
            for rollout_data, final_minibatch, _, _ in \
                ...
                final_value_loss = (self.vf_coef * value_loss) * (rollout_data.returns.size()[0] / self.critic_batch_size)
        ...
        # Unintended advantage: this computes in PyTorch because of the buffer dict swap of internal numpy arrays to tensors
        ...
        if self.policy.policy_optimizer.param_groups[0]['lr'] != 0:
            ...
            while not successful_loop:
                ...
                for epoch in range(self.n_epochs):
                    ...
                    batch = 0
                    np_seed += 1
                    for rollout_data, final_minibatch, adv_mean, adv_std in \
                            self.rollout_buffer.get_minibatch(self.batch_size, self.minibatch_size, np_seed):
                    # for rollout_data in self.rollout_buffer.get(self.batch_size):
                        progress_bar.set_description(f"Training, SPO calc | epoch")
                        ...
                        # advantages = clamp(advantages, min=-3, max=3)
                        ...
                        policy_loss_2 = abs(advantages) * pow(ratio - 1, 2) / (2 * clip_range)
                        policy_loss = -(policy_loss_1 - policy_loss_2).mean()
                        
                        pol_loss_true = -policy_loss_1.mean()
                        pg_losses_true_mini.append(pol_loss_true.item())
                        pol_loss_region = policy_loss_2.mean()
                        pg_losses_region_mini.append(pol_loss_region.item())
                        ...
                        final_policy_loss = (policy_loss + self.ent_coef * entropy_loss) \
                                    * (actions.size()[0] / self.batch_size)
                        ...
                        if final_minibatch:
                            ...
                            pg_losses_true = np_mean(pg_losses_true_mini)
                            pg_losses_region = np_mean(pg_losses_region_mini)
                            ...
                            cut_off = 0.01
                            if pg_loss_mean_batch > cut_off and num_loops < 3 and epoch > 0 and last_pol_loss > pg_loss_mean_batch:
                                ...
                                act_lr = act_lr*0.5
                                print(f"Got policy loss > {cut_off}: {pg_loss_mean_batch}, halving lr to {act_lr}")
                                update_learning_rate(self.policy.policy_optimizer, act_lr)
                                num_loops += 1
                                # this is used in case lowering the lr, in rare cases, causes the loss to increase
                                last_pol_loss = pg_loss_mean_batch
        ...
        # trial run of adjusting lr 
        # pg_losses_true_mean = np_mean(pg_losses_true)
        pg_losses_true_mean = pg_losses_true
        # pg_losses_region_mean = np_mean(pg_losses_region)
        pg_losses_region_mean = pg_losses_region
        
        # kinda a goofy feeling negative for the true pg here but it is correct
        adj_range = 0.03
        if pg_losses_region_mean > -pg_losses_true_mean * 0.45:
            act_lr = act_lr*(1-adj_range)
        if pg_losses_region_mean < -pg_losses_true_mean * 0.25:
            act_lr = act_lr*(1+adj_range)

        # FIXME: this completely overwrites any scheduler that was given by the user (aka potentially bad)
        self.learning_rate = [val_lr, act_lr]
        ...
        if len(pg_losses) != 0:
            ...
            self.logger.record("train/policy_grad_loss_true", pg_losses_true_mean)
            self.logger.record("train/policy_grad_loss_region", pg_losses_region_mean)
            self.logger.record("train/first_policy_grad_loss", pg_losses[0])
            self.logger.record("train/last_policy_grad_loss", pg_losses[-1])
            loss_delta = pg_losses[-1] - pg_losses[0]
            self.logger.record("train/policy_grad_loss_delta", loss_delta)
    ...
    def learn(
        ...
        return super(SPO_Optim, self).learn(
